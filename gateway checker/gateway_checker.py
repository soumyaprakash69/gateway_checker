...
━━━━━━━━━━━━━━
[<b><a href="tg://resolve?domain=washitalr">零</a></b>]𝙋𝙖𝙮𝙢𝙚𝙣𝙩 𝙂𝙖𝙛𝙚𝙡𝙞𝙢𝙞𝙩 -» <code>{gateways_str}</code>
━━━━━━━━━━━━━━
[<b><a href="tg://resolve?domain=washitalr">ᥫ᭡</a></b>]𝘾𝙖𝙥𝙩𝙝𝙖 𝘿𝙚𝙩𝙚𝙘𝙩𝙚𝙙 -» <code>{captcha_detected}</code>
━━━━━━━━━━━━━━
...
# Bulk URL Checking from Text File or Reply
@app.on_message(filters.command(["urltxt"],PREFIXES))
async def urltxt_checker(client: Client, m: Message):
    try:
        # Case 1: Command is replying to a text message
        if m.reply_to_message and m.reply_to_message.text:
            content = m.reply_to_message.text.strip()
            urls = [line.strip() for line in content.split('\n') if line.strip() and is_valid_url(line.strip())]
            
            if not urls:
                await m.reply_text("❌ No valid URLs found in the replied message!", quote=True)
                return
                
            status_msg = await m.reply_text(f"🔍 Found {len(urls)} URLs. Starting check...", quote=True)
            await process_urls(client, m, urls, status_msg)
            return
            
        # Case 2: Command with document attachment
        if m.document:
        ...
        # Case 3: No document or reply
        await m.reply_text("Please either reply to a message containing URLs or upload a .txt file with URLs (one URL per line)", quote=True)
            
    except Exception as e:
        await m.reply_text(f"❌ An error occurred: {str(e)}", quote=True)

# Helper function to process URLs
async def process_urls(client, m, urls, status_msg):
    results = []
...
if __name__ == "__main__":
    try:
        ...
        if "BadMsgNotification" in str(e):
            logger.error("Time synchronization error with Telegram servers. Please check your system time.")
        else:
